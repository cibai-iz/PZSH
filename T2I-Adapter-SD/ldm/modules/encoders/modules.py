...
class FrozenCLIPEmbedder(AbstractEncoder):
    ...
    def __init__(self, version="/hdd/zh/Hash/sd_hash/SD14/A_CLIP/clip-vit-large-patch14", device="cuda", max_length=77,
...
class FrozenCLIPT5Encoder(AbstractEncoder):
    def __init__(self, clip_version="/hdd/zh/Hash/sd_hash/SD14/A_CLIP/clip-vit-large-patch14", t5_version="google/t5-v1_1-xl", device="cuda",
...
class WebUIFrozenCLIPEmebedder(AbstractEncoder):
    def __init__(self, version="/hdd/zh/Hash/sd_hash/SD14/A_CLIP/clip-vit-large-patch14", device="cuda", freeze=True, layer="penultimate"):
        super(WebUIFrozenCLIPEmebedder, self).__init__()
        # self.tokenizer = CLIPTokenizer.from_pretrained(version)  # 原版
        # self.transformer = CLIPModel.from_pretrained(version).text_model  # 原版
        self.tokenizer = CLIPTokenizer.from_pretrained("/hdd/zh/Hash/sd_hash/SD14/A_CLIP/clip-vit-large-patch14")
        self.transformer = CLIPModel.from_pretrained("/hdd/zh/Hash/sd_hash/SD14/A_CLIP/clip-vit-large-patch14").text_model
        self.device = device
        self.layer = layer
        if freeze:
            self.freeze()
